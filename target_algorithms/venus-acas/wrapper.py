...
class PipelineWrapper(AbstractWrapper):
	...
	def get_command_line_args(self, runargs, config):
		...
		'''
		binary_path = "/your/path/here/aclib2/target_algorithms/mip/venus-noCuts-acas/run_venus.py"
		
		with open(runargs["instance"], 'r') as file:
			j = json.loads(file.read())

		cmd = "python3 %s --property acas --acas_prop %s --net %s" %(binary_path, j["acas_prop"], j["net"])
